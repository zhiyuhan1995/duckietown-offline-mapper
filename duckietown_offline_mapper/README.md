...
The reconstruction stage is no longer an IPM fallback. The only supported backend is `vggt_sfm`: VGGT predicts camera parameters and dense depth/point geometry, the backend fuses confident points into a global colored point cloud, optionally exports a COLMAP sparse model / runs bundle adjustment, then the mapping stack fits the ground plane, aligns it to a BEV map frame, segments drivable semantics, projects non-ground obstacle points, inflates obstacles, and exports a ROS-compatible occupancy grid.
...
The UI exposes the requested tabs: Input, Reconstruction, Ground Plane, Alignment, Crop / ROI, BEV, Semantic, Occupancy, and Export.
